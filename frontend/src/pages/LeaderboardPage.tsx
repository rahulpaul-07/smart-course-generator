  ...
  return (
    <div className="page-shell">
      <section className="mb-10 text-center">
        ...
        <p className="mt-2 text-sm text-slate-400">Compete with the community and climb the ranks.</p>
      </section>

      <div className="max-w-4xl mx-auto glass-card rounded-2xl overflow-hidden">
        <div className="p-4 bg-white/5 border-b border-white/10 grid grid-cols-12 gap-4 text-xs font-bold text-slate-400 uppercase tracking-wider hidden sm:grid">
        ...
        <div className="divide-y divide-white/5">
          {leaders.map((user, idx) => (
            <div 
              ...
              className="p-4 grid grid-cols-1 sm:grid-cols-12 gap-4 items-center hover:bg-white/[0.04] transition-colors cursor-pointer group"
            >
              <div className="col-span-2 flex items-center justify-between sm:justify-center">
                <span className="sm:hidden text-xs font-bold text-slate-500 uppercase tracking-wider">Rank</span>
                {idx === 0 ? <Trophy className="h-8 w-8 text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.5)] transform group-hover:scale-110 transition-transform" /> :
                 idx === 1 ? <Trophy className="h-7 w-7 text-slate-300 drop-shadow-md transform group-hover:scale-110 transition-transform" /> :
                 idx === 2 ? <Trophy className="h-6 w-6 text-amber-600 drop-shadow-md transform group-hover:scale-110 transition-transform" /> :
                 <span className="font-display font-bold text-lg text-slate-500">#{idx + 1}</span>}
              </div>
              
              <div className="col-span-5 flex items-center gap-3">
                <div className="h-12 w-12 rounded-full bg-gradient-to-br from-brand-500 to-cyan-500 flex items-center justify-center text-white font-bold shadow-lg overflow-hidden border border-white/10 group-hover:border-brand-400/50 transition-colors">
                  {user.avatar ? (
                ...
                <div>
                  <span className="font-medium text-white group-hover:text-brand-300 transition-colors block">{user.name || 'Anonymous'}</span>
              ...
              <div className="col-span-3 flex flex-wrap items-center gap-2">
                ...
                {(user.achievements?.length || 0) > 4 && (
                  <span className="text-xs text-slate-500">+{user.achievements.length - 4}</span>
                )}
              </div>
              
              <div className="col-span-2 flex items-center justify-between sm:justify-end gap-2 pr-2">
                <span className="sm:hidden text-xs font-bold text-slate-500 uppercase tracking-wider">XP</span>
