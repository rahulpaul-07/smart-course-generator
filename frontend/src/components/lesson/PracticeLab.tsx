...
const PracticeLab = React.memo(({ lessonId, courseId, initialLab = null, embedded = false, onLabUpdate }: { lessonId: string, courseId: string, initialLab?: PracticeLabData | null, embedded?: boolean, onLabUpdate?: (lab: PracticeLabData) => void }) => {
  const [lab, setLab] = useState<PracticeLabData | null>(initialLab);
  ...
  async function generateLab(forceRegenerate = false) {
    ...
    if (fetchError) {
      ...
      const newLab = (data as unknown as { lab: PracticeLabData }).lab;
      setLab(newLab);
      onLabUpdate?.(newLab);
      if (forceRegenerate) {
        setShowSuccess(true);
    ...
    if (embedded) {
      return (
        <div className="flex flex-col items-center gap-3 py-2 text-center">
          ...
          <Button aria-label="Create Practice Lab" disabled={loading} onClick={() => generateLab()} className={`w-full bg-primary hover:bg-primary text-foreground ${loading ? 'cursor-progress' : ''}`}>
    ...
    return (
      <div className="surface-card rounded-2xl p-6">
        <div className="flex flex-col items-center justify-center text-center py-8">
          ...
          <Button aria-label="Create Practice Lab" disabled={loading} onClick={() => generateLab()} size="lg" className={`w-full sm:w-auto bg-primary hover:bg-primary text-foreground ${loading ? 'cursor-progress' : ''}`}>
            {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FlaskConical className="mr-2 h-4 w-4" />}
            {loading ? 'Designing Lab...' : 'Create Practice Lab'}
          </Button>
        </div>
  ...
  return (
    <Card className={`overflow-hidden border-primary/20 ${embedded ? 'border-none shadow-none bg-transparent' : ''}`}>
      <CardHeader className="bg-primary/5 border-b border-border/30 pb-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <Button 
            ...
            disabled={loading}
            className="text-muted-foreground shrink-0 hover:text-primary" 
        ...
        {showSuccess && (
          <div className="mt-4 rounded bg-success/10 p-3 text-sm text-success border border-success/20 flex items-center gap-2">
      ...
      <CardContent className="pt-6 space-y-8">
        ...
        <div>
          <h4 className="text-base font-semibold text-foreground mb-4 flex items-center gap-2">
            <span className="flex h-6 w-6 items-center justify-center rounded bg-success/10 text-xs font-bold text-success">2</span>
            Definition of Done
          </h4>
          <ul className="space-y-2.5 rounded-xl border border-success/20 bg-success/5 p-4">
        ...
        <div className="pt-4 border-t border-border/30">
          ...
          <AnimatePresence>
            {showHint && (
              ...
              >
                <div className="mt-4 rounded-xl border border-warning/20 bg-warning/10 p-4 text-sm text-warning/90 leading-relaxed">
                  {lab.hint}
                </div>
              </motion.div>
            )}
