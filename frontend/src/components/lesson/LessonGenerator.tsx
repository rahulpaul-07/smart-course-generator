    ...
    <section className="relative mb-10 overflow-hidden rounded-2xl bg-card border border-border/50 p-5 shadow-lg animate-enter-delay sm:p-6 group">
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between text-xs font-medium text-brand-300 mb-3 px-1">
            <span className="flex items-center gap-1.5">
              {streamStage === 'Creating outline' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 text-success" />}
              Creating outline
            </span>
            ...
            <span className={`flex items-center gap-1.5 ${streamedCount >= 1 ? 'text-brand-300' : 'text-muted-foreground'}`}>
              {streamStage === 'Writing section' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (streamedCount >= 1 ? <Sparkles className="w-3.5 h-3.5 text-success" /> : <span className="w-3.5 h-3.5" />)}
              Writing section
            </span>
            <span className="text-muted-foreground hidden sm:inline">&rarr;</span>
            <span className={`flex items-center gap-1.5 ${streamedCount >= 3 ? 'text-brand-300' : 'text-muted-foreground'}`}>
              {streamStage === 'Generating code examples' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (streamedCount >= 3 ? <Sparkles className="w-3.5 h-3.5 text-success" /> : <span className="w-3.5 h-3.5" />)}
              Code examples
            </span>
            <span className="text-muted-foreground hidden sm:inline">&rarr;</span>
            <span className={`flex items-center gap-1.5 ${streamedCount >= 6 ? 'text-brand-300' : 'text-muted-foreground'}`}>
              {streamStage === 'Finalizing lesson' || streamStage === 'Saving lesson' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (streamStage === 'Saving lesson' ? <Sparkles className="w-3.5 h-3.5 text-success" /> : <span className="w-3.5 h-3.5" />)}
              Finalizing
            </span>
          </div>
      ...
      {isPickerOpen && (
        <div className="relative mt-6 grid gap-5 rounded-2xl border border-border bg-muted/40 p-5 sm:grid-cols-2">
          ...
          <label className="text-sm text-foreground/90">
            <span className="flex items-center gap-2 font-medium"><Languages className="h-4 w-4 text-primary" /> Language</span>
            <input
              type="text"
              value={language}
