  ...
  return (
    <article data-reading-content className={`mx-auto w-full transition-all duration-300 px-5 py-12 lg:py-16 ${isFocusMode ? 'max-w-[1400px] lg:px-16' : 'max-w-[820px] lg:px-12'}`}>
      ...
      {streamStatus === 'interrupted' && (
        <div className="bg-warning/10 border border-warning/20 p-8 rounded-2xl my-8 flex flex-col items-center justify-center text-center shadow-sm backdrop-blur-sm">
          ...
          <div className="flex gap-4">
            <button onClick={() => window.location.reload()} className="bg-background border border-border text-foreground px-6 py-2.5 rounded-xl font-bold shadow-sm hover:shadow-md transition-all">Reload Page</button>
      ...
      {!hasContent && !generating && streamStatus !== 'error' && (
        <div className="flex flex-col items-center justify-center py-24 text-center rounded-2xl border-2 border-dashed border-border bg-card mt-12 shadow-sm">
          <div className="h-24 w-24 bg-muted border border-border rounded-[2rem] flex items-center justify-center mb-6 shadow-inner">
