...
export function LessonLayout({
  ...
  return (
    <div className={`flex overflow-hidden relative bg-background transition-all duration-300 ease-out ${isFocusMode ? 'fixed inset-0 z-50 h-screen w-screen flex-col' : 'h-full flex-row'}`}>
      ...
      <div
        ref={lessonScrollRef}
        className={`flex-1 min-h-0 min-w-0 overflow-y-auto scroll-smooth relative bg-background pb-24 lg:pb-0 transition-[margin-right] duration-200 ${isFocusMode ? 'px-4 sm:px-8' : ''} ${toolsAvailable && toolsOpen ? 'lg:mr-[380px] xl:mr-[400px]' : ''}`}
      >
        {children}
      </div>
