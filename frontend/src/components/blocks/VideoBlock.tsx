  ...
  if (!id && !isSearch) {
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="block p-5 rounded-xl border border-border bg-card/50 shadow-sm text-primary transition-all hover:border-primary/40 hover:shadow-md"
      >
    ...
    <div className="my-8 overflow-hidden rounded-xl border border-border bg-card">
      <div>
        <iframe
          ...
          src={isSearch ? `https://www.youtube.com/embed?listType=search&list=${encodeURIComponent(searchQuery || "")}` : `https://www.youtube.com/embed/${encodeURIComponent(id || "")}`}
