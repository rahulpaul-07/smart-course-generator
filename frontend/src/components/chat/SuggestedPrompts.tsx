      ...
      <div className="w-full space-y-2.5">
        ...
        {SUGGESTIONS.map((s, i) => (
          <button
            key={i}
            onClick={() => onSelect(s)}
            className="w-full text-left px-5 py-3 rounded-xl border border-border bg-muted/20 text-[13px] font-medium text-foreground hover:bg-primary/5 hover:border-primary/30 hover:text-primary transition-all duration-200 shadow-sm"
          >
            {s}
          </button>
        ))}
