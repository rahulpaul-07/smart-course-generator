  ...
  return (
    ...
    >
      <Card className="p-6 rounded-2xl border border-border bg-card shadow-sm">
        ...
        {skills.length === 0 ? (
          <p className="text-sm text-muted-foreground">Not specified for this course.</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {skills.map((skill, i) => (
              <div key={i} className="inline-flex items-center gap-2 bg-background border border-border rounded-lg px-3 py-2 shadow-sm transition-colors hover:border-border">
