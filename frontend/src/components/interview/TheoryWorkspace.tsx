  ...
  return (
    <div className="space-y-10">
      ...
      {prep.theoryQuestions?.map((q: TheoryQuestion, i: number) => {
        ...
        return (
          <div key={i} className="rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-sm flex flex-col gap-6">
            ...
            <div className="relative pl-12">
              <textarea
                ...
                disabled={submitted}
                rows={8}
                className="w-full rounded-2xl border border-border bg-background/50 p-5 text-[15px] leading-relaxed text-foreground outline-none transition focus:border-primary/50 focus:ring-4 focus:ring-primary/10 shadow-inner resize-y disabled:opacity-70 disabled:cursor-not-allowed placeholder:text-muted-foreground"
                placeholder="Structure your answer clearly. Consider using bullet points or real-world examples..."
              />
              {!submitted && (
                <div className="absolute bottom-4 right-4 flex items-center gap-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground bg-background/80 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-border shadow-sm pointer-events-none">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 ml-12 space-y-5 rounded-2xl border border-border bg-card p-6 shadow-sm">
                ...
                {q.idealAnswer && (
                  <details className="group mt-6 border border-border rounded-xl bg-background/50 overflow-hidden">
                    ...
                    <div className="p-5 border-t border-border text-[14px] leading-relaxed text-muted-foreground bg-background">
                      {q.idealAnswer}
                    </div>
                  </details>
                )}
